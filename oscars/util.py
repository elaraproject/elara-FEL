...
def read_file_list_with_header (ifile, idir=None):
    ...
    mapping: mapping object
        Mapping of parameters and filenames along with header infomation as members
    """
...
def read_file_list_2d_with_header (ifile, idir=None):
    """
    read a list of parameters and filenames from a file.
    ...
    In the first column is the gap value as a floating point number
    and in the second column the phase mode, thrd, the phase as a float,
    and fourth column the file name.  Whitespace in the name is
    unforgivably wrong, but technically allowed.
...
def interpolate_file_list_2d (mapping, gap, phase, ofile, idir=None, tmpdir=None):
    """
    read a list of parameters and filenames from mapping.  Interpolates across phases for each gap to get
    ...
    ----------
    mapping : mapping
        Mapping containing format parameters and list of parameters with filenames
    ...
    for key in phases_at_gap:
        ...
        osr.clear_bfields()
        osr.add_bfield_interpolated(mapping=phases_at_gap[key], iformat=mapping.format, parameter=phase, ofile=tmp_file_name)

    osr.clear_bfields()
    osr.add_bfield_interpolated(mapping=new_mapping, iformat=mapping.format, parameter=gap, ofile=ofile)

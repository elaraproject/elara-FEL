...
fluxcorrectionarray = np.loadtxt(resource_stream(__name__,'data/srw/brilliance/gwSrwBrilUndHarmUnivFlux.txt'))
divcorrectionarray = np.loadtxt(resource_stream (__name__,'data/srw/brilliance/gwSrwBrilUndHarmUnivDiv.txt'))
sizecorrectionarray = np.loadtxt(resource_stream(__name__,'data/srw/brilliance/gwSrwBrilUndHarmUnivSize.txt'))
...
def srw_epu_brightness(oth, Kx_range, Ky_range, period, length, npoints=1000, detuning=0):
    ...
    # print('beam_energy_GeV      ', beam_energy_GeV)
    # print('beam_energy_sigma_rel', beam_energy_sigma_rel)
    ...
    nPer = int(length / period) # undulator.length / undulator.period

    Kx_1 = 1e-2 if Kx_range[1] == 0 else Kx_range[1]
    ...
    (x, y) = srwl_und_bright_en(
        current, # electronBeam.current
        Ky_range[0], # undulator.horizontalDeflectingParameter
        Kx_range[0], # undulator.verticalDeflectingParameter
        -0.7853981633974483, # undulator.horizontalInitialPhase
        +0.7853981633974483, # undulator.verticalInitialPhase
        1, # harmonic
        beam_energy_GeV, # electronBeam.energy
        ...
        sigma_x[1] ** 2, # electronBeam.rmsSizeY
        sigma_xp[0] ** 2, # electronBeam.rmsDivergX
        sigma_xp[1] ** 2, # electronBeam.rmsDivergY
        Ky_range[0], # undulator.horizontalDeflectingParameter
        Kx_range[0], # undulator.verticalDeflectingParameter
        Kx_1, # brillianceReport.minDeflection
        npoints, # brillianceReport.energyPointCount
    )
...
def srw_und_brightness(oth, Kx_range, period, length, harmonic, npoints=1000, detuning=0):
    ...
    if harmonic % 2:
        ...
        (x, y) = srwl_und_bright_en(
            current, # electronBeam.current
            0.0, # undulator.horizontalDeflectingParameter
            Kx_range[0], # undulator.verticalDeflectingParameter
            0.0, # undulator.horizontalInitialPhase
            0.0, # undulator.verticalInitialPhase
            harmonic,
            beam_energy_GeV, # electronBeam.energy
            lam_u, # undulator period in cm
            nPer,
...
def srw_epu_flux(oth, Kx_range, Ky_range, period, length, npoints=1000):
    beam_energy_GeV = oth.get_beam_energy()
    beam_energy_sigma_rel = oth.get_beam_energy_sigma() / beam_energy_GeV
    current = oth.get_beam_current()
    ...
    nPer = int(length / period) # undulator.length / undulator.period
    lam_u = period * 100.0      # period in cm

    flux = []
    (x, y, z) = srwl_und_flux_en(
            current,                # Ib
            Ky_range[0],            # kxmax
            Kx_range[0],            # kzmax
            0.1,                    # kmin
...
def srw_epu_flux_onaxis(oth, Kx_range, Ky_range, period, length, npoints=1000):
    beam_energy_GeV = oth.get_beam_energy()
    beam_energy_sigma_rel = oth.get_beam_energy_sigma() / beam_energy_GeV
    current = oth.get_beam_current()
    emittance = oth.get_emittance()
    ...
    (x, y) = srwl_und_ang_flux_en(
            ...
            0.0,                    # enDetPar
            beam_energy_sigma_rel,  # relEnSpr
            sigma_xp[0]**2,         # sigpxsq
            sigma_xp[1]**2,         # sigpzsq
            )
...
def srw_und_flux_onaxis(oth, Kx_range, period, length, harmonic, npoints=1000):
    beam_energy_GeV = oth.get_beam_energy()
    beam_energy_sigma_rel = oth.get_beam_energy_sigma() / beam_energy_GeV
    current = oth.get_beam_current()
    emittance = oth.get_emittance()
